...
def transcribe_and_delete_entries():
    try:
        ...
        translated_text = translator.translate(spoken_text, src='hi', dest='en').text
        print(f"Translated Text: {translated_text}")

        # Iterate through rows to find matching entries and delete them
        rows_to_delete = []
        for row in sheet.iter_rows(min_row=2, max_row=sheet.max_row, min_col=1, max_col=3):
            first_name_hi = row[0].value
            last_name_hi = row[1].value
            if first_name_hi and last_name_hi:
                first_name_en = translator.translate(first_name_hi, src='hi', dest='en').text
                last_name_en = translator.translate(last_name_hi, src='hi', dest='en').text
...
if __name__ == "__main__":
    print("SpeechDBQuery - Delete Entries in Hindi")
